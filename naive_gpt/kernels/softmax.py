...
class MaskedSoftmax(autograd.Function):
    @staticmethod
    def forward(ctx,
                indptr: torch.Tensor,
                indices: torch.Tensor,
                values: torch.Tensor):
        output = ext.masked_softmax_forward_cuda(
            indptr, indices, values
        )
        ctx.save_for_backward(
            indptr, indices, output
    ...
    def backward(ctx,
        ...
        indices = ctx.saved_tensors[1]
        output = ctx.saved_tensors[2]
        grad_values = ext.masked_softmax_backward_cuda(
            indptr, indices, output, grad_output.contiguous()
        )
        return None, None, grad_values
