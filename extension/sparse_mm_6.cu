...
__global__ void sddmm_forward_cuda_kernel(
    ...
    // cache lhs
    __shared__ vector_t cache_lhs[BSZ][BSZ / TSZ];
    cache_lhs[ty][tx] = __ldg(
    ...
    for (index_t i = indptr[gy]; i < indptr[gy + 1]; i += BSZ) {
        ...
        for (index_t t = 0; t < TSZ; t += 1) {
            ...
            for (index_t k = 0; k < BSZ; k += 4) {
                ...
                reduced[t] += cache_lhs[ty][k / TSZ].x * cache_rhs.x;
                reduced[t] += cache_lhs[ty][k / TSZ].y * cache_rhs.y;
                reduced[t] += cache_lhs[ty][k / TSZ].z * cache_rhs.z;
                reduced[t] += cache_lhs[ty][k / TSZ].w * cache_rhs.w;
            }
        }
