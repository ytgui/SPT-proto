...
class RoutedLinear(nn.Linear):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for b, items in enumerate(indices):
            for col, row_list in enumerate(items):
                for row in row_list:
                    if (row, col) not in grouping:
                        grouping[row, col] = []
                    grouping[row, col].append(b)

        # block-wise visit
        y = torch.zeros(
            [x.size(0), self.out_blocks, self.block_size],
            device=x.device
        )
        for (row, col), batches in grouping.items():
            x_i = x[batches, col]
            w_i = self.weight[
                row * self.block_size:(row + 1) * self.block_size,
                col * self.block_size:(col + 1) * self.block_size,
            ]
            y[batches, row] = torch.addmm(
                y[batches, row], x_i, w_i.T, beta=1.0, alpha=1.0
            )
        y = y.view(
